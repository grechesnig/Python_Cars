class Car:
    ...
    def __init__(self, firm, series, number, price, year, month, color, engine_volume) -> None:
        self.firm: str = firm
...
def save_to_file(info):
    file = open(input('Введите название файла для записи результатов: '), 'w')
    for i in info:
        file.write(i)
    ...
    for car in car_list:
        ...
        if car.color == color:
            ...
            for car2 in car_list:
                if car != car2 and car2.firm == car.firm and car2.color != color:
                    flag = 0
                    break
